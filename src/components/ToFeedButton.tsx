...
const ToFeedButton = () => {
  ...
  const subbreaditPath = getsubbreaditPath(pathname);

  return (
    <a href={subbreaditPath} className={buttonVariants({ variant: "ghost" })}>
      <ChevronLeft className="h-4 w-4 mr-1" />
      {subbreaditPath === "/" ? "Back home" : "Back to community"}
    </a>
  );
};

const getsubbreaditPath = (pathname: string) => {
